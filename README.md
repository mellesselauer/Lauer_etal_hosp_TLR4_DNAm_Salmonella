...
Supplemental methods document containing: variable mapping table (Table S1) linking R variable names to manuscript labels; full model comparisons, full model specifications with formal mathematical notation; model fitting, slope extraction, repeated measures correlations `rmcorr` and `emmeans` contrast code; convergence and posterior predictive check figures (Figures S1–S20); and supplemental model output tables (Tables S2–S7).
...
- Model objects (`fit_cgslopes` and `fit_`) are not deposited due to file size. Run the .Rmd files to reproduce all results.
